...
class TestBGPCommon:
    ...
    def ft_bgp_peer_traffic_check(self):
        ...
        if not utils.poll_wait(bgplib.l3tc_vrfipv4v6_address_leafspine_bgp_check, 20, config_type='ipv4'):
            st.error("Neighbor is failed to Establish between Spine - Leaf after clear ip bgp")
            utils.exec_all(True, [[st.generate_tech_support, topo.dut_list[0], 'ft_bgp_peer_traffic_check'], [st.generate_tech_support, topo.dut_list[1], 'ft_bgp_peer_traffic_check']])
            tc_fail_flag = 1
        bgp_summary_spine_before_timer = bgpapi.show_bgp_ipv4_summary(topo.dut_list[1])
        rib_entries_before_update_timer = bgp_summary_spine_before_timer[0]['ribentries']
    ...
    def ft_bgp_graceful_restart_and_aware_routers(self):
        ...
        result = bgpapi.verify_bgp_summary(leaf_name, family='ipv4', neighbor=info['D1D2P1_ipv4'], state='Established' , shell=bgp_cli_type)
        if not result:
            utils.exec_all(True, [[st.generate_tech_support, leaf_name, 'ft_bgp_graceful_restart_and_aware_routers'], [st.generate_tech_support, spine_name, 'ft_bgp_graceful_restart_and_aware_routers']])
    ...
    def ft_bgp_ipv4_no_route_aggregation_for_exact_prefix_match(self):
        ...
        result = bgpapi.verify_bgp_summary(leaf_name, family='ipv4', neighbor=info['T1D2P1_ipv4'], state='Established', shell=bgp_cli_type)
    ...
    def ft_bgp_ipv6_route_aggregation_with_as_set(self):
        ...
        # Verify bgp neighbors between leaf and Tg
        result = bgpapi.verify_bgp_summary(leaf_name, family='ipv6', neighbor=info['T1D2P1_ipv6'], state='Established', shell=bgp_cli_type)
...
class TestBGPRif(TestBGPCommon):
    ...
    def test_ft_bgp_rmap(self):
        ...
        bgpapi.advertise_bgp_network(leaf_name, info['D2_as'], network1, cli_type=vtysh_cli_type)
        ...
        if not n1:
            ...
            utils.exec_all(True, [[st.generate_tech_support, leaf_name, 'ft_bgp_rmap'], [st.generate_tech_support, spine_name, 'ft_bgp_rmap']])
            result = False
        # Clear applied configs
    ...
    def test_ft_bgp_rmap_out(self, bgp_rif_func_hook):
        ...
        n2 = utils.poll_wait(ipapi.verify_ip_route,5,spine_name,ip_address=network2)
        if n2:
            result = result & True
        else:
            result = result & False
...
class TestBGPIPvxRouteAdvertisementFilter:
    ...
    def test_redistribute_connected_ipv4(self, bgp_ipvx_route_adv_filter_fixture):
        ...
        if set(list_of_connected_network_on_dut1).issubset(set(list_of_learned_routes_on_dut2_from_dut1)):
            ...
            utils.exec_all(True, [[st.generate_tech_support, self.local_topo['dut1'], 'ft_redistribute_connected_ipv4'], [st.generate_tech_support, self.local_topo['dut2'], 'ft_redistribute_connected_ipv4']])
            result = False
    ...
    def test_redistribute_static_ipv4(self, bgp_ipvx_route_adv_filter_fixture):
        ...
        if set(list_of_static_network_on_dut1).issubset(set(list_of_learned_routes_on_dut2_by_dut1)):
            ...
            st.error('static on dut1 is not a subset of dut1 learned route on dut2')
            utils.exec_all(True, [[st.generate_tech_support, self.local_topo['dut1'], 'ft_redistribute_static_ipv4'], [st.generate_tech_support, self.local_topo['dut2'], 'ft_redistribute_static_ipv4']])
            result = False
    ...
    def test_distribute_list_in_ipv4(self, bgp_ipvx_route_adv_filter_fixture):
        ...
        list_of_learned_routes_on_dut2_by_dut1 = list(x['network'] for x in output)
        if '102.1.1.0/24' in list_of_learned_routes_on_dut2_by_dut1:
            st.error("route not suppressed")
            utils.exec_all(True, [[st.generate_tech_support, self.local_topo['dut1'], 'ft_distribute_list_in_ipv4'], [st.generate_tech_support, self.local_topo['dut2'], 'ft_distribute_list_in_ipv4']])
            result = False
        else:
    ...
    def test_filter_list_in_ipv4(self, bgp_ipvx_route_adv_filter_fixture):
        ...
        if len(list_of_learned_routes_on_dut2_by_dut1) != 0:
            st.error("still having routes from as {}".format(self.local_topo['dut1_as']))
            utils.exec_all(True, [[st.generate_tech_support, self.local_topo['dut1'], 'ft_filter_list_in_ipv4'], [st.generate_tech_support, self.local_topo['dut2'], 'ft_filter_list_in_ipv4']])
            result = False
        else:
    ...
    def test_prefix_list_out_ipv4(self, bgp_ipvx_route_adv_filter_fixture):
        ...
        if '202.1.1.0/24' in list_of_learned_routes_on_dut1_by_dut2:
            st.error("route not suppressed")
            utils.exec_all(True, [[st.generate_tech_support, self.local_topo['dut1'], 'ft_prefix_list_out_ipv4'], [st.generate_tech_support, self.local_topo['dut2'], 'ft_prefix_list_out_ipv4']])
            result = False
        else:
            st.log("route suppressed")
            result = True
    ...
    def test_default_originate_ipv4(self, bgp_ipvx_route_adv_filter_fixture):
        ...
        if '0.0.0.0/0' in list_of_learned_routes_on_dut1_by_dut2:
            ...
            st.error("default route not advertised")
            utils.exec_all(True, [[st.generate_tech_support, self.local_topo['dut1'], 'ft_default_originate_ipv4'], [st.generate_tech_support, self.local_topo['dut2'], 'ft_default_originate_ipv4']])
            result = False
    ...
    def test_redistribute_connected_ipv6(self, bgp_ipvx_route_adv_filter_fixture):
        ...
        if set(list_of_connected_network_on_dut1).issubset(set(list_of_learned_routes_on_dut2_by_dut1)):
            ...
            utils.exec_all(True, [[st.generate_tech_support, self.local_topo['dut1'], 'ft_redistribute_connected_ipv6'], [st.generate_tech_support, self.local_topo['dut2'], 'ft_redistribute_connected_ipv6']])
            result = False
    ...
    def test_redistribute_static_ipv6(self, bgp_ipvx_route_adv_filter_fixture):
        ...
        if set(list_of_static_network_on_dut1).issubset(set(list_of_learned_routes_on_dut2_by_dut1)):
            ...
            st.error('static on dut1 is not a subset of dut1 learned route on dut2')
            utils.exec_all(True, [[st.generate_tech_support, self.local_topo['dut1'], 'ft_redistribute_static_ipv6'], [st.generate_tech_support, self.local_topo['dut2'], 'ft_redistribute_static_ipv6']])
            result = False
    ...
    def test_distribute_list_in_ipv6(self, bgp_ipvx_route_adv_filter_fixture):
        ...
        if '102:1::/64' in list_of_learned_routes_on_dut2_by_dut1:
            st.error("route not suppressed")
            utils.exec_all(True, [[st.generate_tech_support, self.local_topo['dut1'], 'ft_distribute_list_in_ipv6'], [st.generate_tech_support, self.local_topo['dut2'], 'ft_distribute_list_in_ipv6']])
            result = False
        else:
            st.log("route suppressed")
            result = True
    ...
    def test_filter_list_in_ipv6(self, bgp_ipvx_route_adv_filter_fixture):
        ...
        if len(list_of_learned_routes_on_dut2_by_dut1) != 0:
            st.error("still having routes from as {}".format(self.local_topo['dut1_as']))
            utils.exec_all(True, [[st.generate_tech_support, self.local_topo['dut1'], 'ft_filter_list_in_ipv6'], [st.generate_tech_support, self.local_topo['dut2'], 'ft_filter_list_in_ipv6']])
            result = False
        else:
            st.log("no routes from as {}".format(self.local_topo['dut1_as']))
            result = True
    ...
    def test_prefix_list_out_ipv6(self, bgp_ipvx_route_adv_filter_fixture):
        ...
        if '202:1::/64' in list_of_learned_routes_on_dut1_by_dut2:
            st.error("route not suppressed")
            utils.exec_all(True, [[st.generate_tech_support, self.local_topo['dut1'], 'ft_prefix_list_out_ipv6'], [st.generate_tech_support, self.local_topo['dut2'], 'ft_prefix_list_out_ipv6']])
            result = False
        else:
            st.log("route suppressed")
            result = True
    ...
    def test_filter_list_out_ipv6(self, bgp_ipvx_route_adv_filter_fixture):
        ...
        list_of_learned_routes_on_dut2_by_dut1 = list(x['network'] for x in output)
        if len(list_of_learned_routes_on_dut2_by_dut1) != 0:
            st.error("still having routes from as {}".format(self.local_topo['dut2_as']))
            utils.exec_all(True, [[st.generate_tech_support, self.local_topo['dut1'], 'ft_filter_list_out_ipv6'], [st.generate_tech_support, self.local_topo['dut2'], 'ft_filter_list_out_ipv6']])
            result = False
        else:
    ...
    def test_default_originate_ipv6(self, bgp_ipvx_route_adv_filter_fixture):
        ...
        if '::/0' in list_of_learned_routes_on_dut1_by_dut2:
            ...
            st.error("default route not advertised")
            utils.exec_all(True, [[st.generate_tech_support, self.local_topo['dut1'], 'ft_default_originate_ipv6'], [st.generate_tech_support, self.local_topo['dut2'], 'ft_default_originate_ipv6']])
            result = False
    ...
    def test_bgp_ebgp6_traffic(self, bgp_ipvx_route_adv_filter_fixture):
        ...
        if int(rib_entries_after_update_timer) < 500:
            st.error('Routes are not advertised to peer DUT, even after the update delay timer expiry')
            utils.exec_all(True, [[st.generate_tech_support, topo.dut_list[0], 'ft_bgp_ebgp6_traffic'], [st.generate_tech_support, topo.dut_list[1], 'ft_bgp_ebgp6_traffic']])
            tc_fail_flag = 1
    ...
    def test_route_aggregate_ipv6(self, bgp_ipvx_route_adv_filter_fixture):
        ...
        bgpapi.fetch_ip_bgp_route(self.local_topo['dut1'], family='ipv6')
        output = bgpapi.fetch_ip_bgp_route(self.local_topo['dut2'], family='ipv6',
        ...
        if (aggr_addr in list_of_learned_routes_on_dut2_by_dut) and aggregation:
            ...
            utils.exec_all(True, [[st.generate_tech_support, self.local_topo['dut1'], 'ft_route_aggregate_ipv6'], [st.generate_tech_support, self.local_topo['dut2'], 'ft_route_aggregate_ipv6']])
            result = False
    ...
    def test_static_blackhole_rt_redistribute_with_routemap_ipv6(self, bgp_ipvx_route_adv_filter_fixture):
        ...
        if metric == '50':
            ...
            utils.exec_all(True, [[st.generate_tech_support, self.local_topo['dut1'], 'ft_static_blackhole_rt_redistribute_with_routemap_ipv6'], [st.generate_tech_support, self.local_topo['dut2'], 'ft_static_blackhole_rt_redistribute_with_routemap_ipv6']])
            result = False
